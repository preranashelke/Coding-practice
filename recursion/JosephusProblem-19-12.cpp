...
//person is killed in circle. The task is to choose the place in the initial circle so that you are the last
// one remaining and so survive.
//Consider if n = 5 and k = 2, then the safe position is 3.
//Firstly, the person at position 2 is killed, then person at position 4 is killed, then person at position 1
// is killed. Finally, the person at position 5 is killed. So the person at position 3 survives.
int jos(int n,int k){
if(n==1){
    return 0;
}
else{
return (jos(n-1,k)+k)%n;  //modulo to maintain circular manner
}
}

int jos1(int n,int k){  //starting from 1
